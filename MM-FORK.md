...
Built off `upstream/main` at `d78bf58` (mlx-lm 0.32.0 line, "Support for
...
| (tip) | Version marker `0.32.0+mm.2` | *integration-only, never submitted* | n/a | Lets `pip list` / `uv pip list` distinguish a fork install from a PyPI install on any node. |
...
```toml
mlx-lm = { git = "https://github.com/azamamirza/mlx-lm.git", tag = "mm-v0.32.0-mm.2" }
```
...
uv pip install --python ~/mlx-mesh/bin/python --no-deps --reinstall-package mlx-lm \
  "mlx-lm @ git+https://github.com/azamamirza/mlx-lm.git@mm-v0.32.0-mm.2"
```

Confirm with `uv pip list --python ~/mlx-mesh/bin/python | grep mlx-lm` —
it must read `0.32.0+mm.2`. A bare `0.32.0` means the node is on PyPI and is
missing `shard()`.
...
4. Rebuild the integration branch from the (rebased) code commits:
   ...
   #   0.32.0+mm.2 -> <new upstream version>+mm.<n>
   ```
