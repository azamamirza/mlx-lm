...
Built off `upstream/main` at `e5baded` (mlx-lm 0.31.3 line, "Support for
Poolside LagunaXS ... (#1334)").

| Commit | Fix | Submission branch | Upstream PR | Why the fleet needs it |
|--------|-----|-------------------|-------------|------------------------|
| `2bd04c7` | Fix `ChunkedKVCache.maybe_trim_front` to trim on the valid length | `fix-chunked-kv-trim` | not yet filed — ledger candidate 1 | Correctness bug: `start_position` drifts (252 vs 2) after a front trim. Affects every long-context / prompt-cache path. |
| `a4b3a3d` | Tensor parallel sharding for Qwen3 MoE (`Model.shard()`) | `qwen3-moe-tensor-parallel` | not yet filed — ledger candidate 2 | **Load-bearing.** Without it the 2-node Qwen3-235B-A22B-4bit TP ring cannot run at all. |
| `ab109fa` | Pipeline parallelism for Qwen3 MoE (`PipelineMixin`, `pipeline_layers`) | `qwen3-moe-pipeline` | not yet filed — ledger candidate 3 | Enables the PP lane for the same checkpoint. Measured 0.87x of TP, so TP stays the quality lane, but the code path must exist. |
| (tip) | Version marker `0.31.3+mm.1` | *integration-only, never submitted* | n/a | Lets `pip list` / `uv pip list` distinguish a fork install from a PyPI install on any node. |
...
```toml
mlx-lm = { git = "https://github.com/azamamirza/mlx-lm.git", tag = "mm-v0.31.3-mm.1" }
```
...
uv pip install --python ~/mlx-mesh/bin/python --no-deps --reinstall-package mlx-lm \
  "mlx-lm @ git+https://github.com/azamamirza/mlx-lm.git@mm-v0.31.3-mm.1"
```

Confirm with `uv pip list --python ~/mlx-mesh/bin/python | grep mlx-lm` —
it must read `0.31.3+mm.1`. A bare `0.31.3` means the node is on PyPI and is
missing `shard()`.
...
4. Rebuild the integration branch from the (rebased) code commits:
   ...
   #   0.31.3+mm.1 -> <new upstream version>+mm.1
   ```
