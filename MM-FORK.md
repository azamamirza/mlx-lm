...
Built off `upstream/main` at `e93eaf7` (mlx-lm 0.32.0 line — the tip IS our
own merged #1801, "Add pipeline parallelism for Qwen3 MoE").

| Commit | Fix | Submission branch | Upstream PR | Why the fleet needs it |
|--------|-----|-------------------|-------------|------------------------|
| — | ~~Tensor parallel sharding for Qwen3 MoE~~ | `qwen3-moe-tensor-parallel` | **MERGED upstream — ml-explore/mlx-lm#1774 (2026-08-22)** | **REMOVED from this branch 2026-09-01**: the fix now arrives from upstream itself. First upstream contribution landed. |
| — | ~~Pipeline parallelism for Qwen3 MoE~~ | `qwen3-moe-pipeline` | **MERGED upstream — ml-explore/mlx-lm#1801 (2026-08-29, commit e93eaf7)** | **REMOVED from this branch 2026-09-01**: upstream/main's tip is this fix. |
...
| (tip) | Version marker `0.32.0+mm.4` | *integration-only, never submitted* | n/a | Lets `pip list` / `uv pip list` distinguish a fork install from a PyPI install on any node. |
...
```toml
mlx-lm = { git = "https://github.com/azamamirza/mlx-lm.git", tag = "mm-v0.32.0-mm.4" }
```
...
uv pip install --python ~/mlx-mesh/bin/python --no-deps --reinstall-package mlx-lm \
  "mlx-lm @ git+https://github.com/azamamirza/mlx-lm.git@mm-v0.32.0-mm.4"
```

Confirm with `uv pip list --python ~/mlx-mesh/bin/python | grep mlx-lm` —
it must read `0.32.0+mm.4`. A bare `0.32.0` means the node is on PyPI and is
missing `shard()`.
...
4. Rebuild the integration branch from the (rebased) code commits:
   ...
   #   0.32.0+mm.4 -> <new upstream version>+mm.<n>
   ```
...
Expected on the current FLEET pin: `0.31.3+mm.1 True True`. On the new
`mm-v0.32.0-mm.4` tag: `0.32.0+mm.4 True True` — and note that from this tag
onward `shard()`/`pipeline()` exist in UPSTREAM releases too (#1774/#1801
merged), so `hasattr` alone no longer distinguishes fork from PyPI; the `+mm`
version marker is the discriminator.

## Milestone (2026-09-01): zero functional divergence

With #1774 and #1801 merged upstream and #1673 having superseded the trim fix,
`mm-integration` carries **no code patches at all** — it is `upstream/main`
plus this document and the version marker. The fork's whole purpose from here
is (a) an immutable-tag install channel the fleet controls, and (b) a staging
ground for future candidates (next: uneven pipeline splits, staged as a patch
in `bridge/upstream/staged/`).
